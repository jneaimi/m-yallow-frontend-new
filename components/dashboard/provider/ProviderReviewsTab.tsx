          ...
          <div className="md:w-3/4">
            <div className="space-y-3">
              {[5, 4, 3, 2, 1].map((rating) => {
                ...
                const reviewsWithThisRating = reviews?.filter(review => review.rating === rating).length || 0;
                const percent = reviewCount > 0 
                  ? Math.round((reviewsWithThisRating / reviewCount) * 100) 
                  : 0;
                return (
                  <div key={rating} className="flex items-center gap-3">
                    <div className="flex items-center gap-1 w-16">
