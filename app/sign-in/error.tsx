...
export default function SignInError({
  ...
  useEffect(() => {
    ...
    if (announcer) {
      announcer.textContent = "An error occurred during sign in. " + error.message;
    }
  }, [error]);
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        >
          ...
          <div className="space-y-3 w-full">
            <div>
              <h2 className="font-semibold text-destructive">Authentication Error</h2>
              <p className="text-sm text-muted-foreground mt-1">{error.message || "An error occurred during sign in."}</p>
            </div>
            <Button 
              variant="outline" 
              onClick={reset}
